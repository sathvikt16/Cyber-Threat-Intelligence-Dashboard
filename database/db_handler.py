...
# # def init_db():
# #     """Initializes the database with schema for embeddings and dynamic sources."""
# #     with sqlite3.connect(DATABASE_NAME) as conn:
# #         cursor = conn.cursor()
# #         cursor.execute('''
# #             CREATE TABLE IF NOT EXISTS pulses (
# #                 id INTEGER PRIMARY KEY AUTOINCREMENT, source TEXT NOT NULL, title TEXT NOT NULL UNIQUE,
# #                 url TEXT, threat_name TEXT, threat_category TEXT, targeted_industries TEXT,
# #                 targeted_countries TEXT, summary TEXT, published_at DATETIME,
# #                 embedding BLOB,
# #                 collection_timestamp DATETIME DEFAULT CURRENT_TIMESTAMP
# #             )
# #         ''')
...
# #                 value TEXT NOT NULL UNIQUE, enrichment_data TEXT
# #             )
# #         ''')
# #         cursor.execute('''
# #             CREATE TABLE IF NOT EXISTS pulse_indicators (
# #                 pulse_id INTEGER, indicator_id INTEGER,
# #                 FOREIGN KEY(pulse_id) REFERENCES pulses(id),
# #                 FOREIGN KEY(indicator_id) REFERENCES indicators(id),
# #                 PRIMARY KEY (pulse_id, indicator_id)
# #             )
# #         ''')
# #         cursor.execute('''
# #             CREATE TABLE IF NOT EXISTS pulse_correlations (
# #                 source_pulse_id INTEGER, related_pulse_id INTEGER, reason TEXT,
# #                 FOREIGN KEY(source_pulse_id) REFERENCES pulses(id),
# #                 FOREIGN KEY(related_pulse_id) REFERENCES pulses(id),
# #                 PRIMARY KEY (source_pulse_id, related_pulse_id)
# #             )
# #         ''')
# #         cursor.execute('''
...
# # def get_correlations_for_pulse(pulse_id):
# #     with sqlite3.connect(DATABASE_NAME) as conn:
# #         conn.row_factory = sqlite3.Row; cursor = conn.cursor()
# #         cursor.execute('SELECT p.id, p.title, p.source, c.reason FROM pulse_correlations c JOIN pulses p ON c.related_pulse_id = p.id WHERE c.source_pulse_id = ? ORDER BY p.published_at DESC', (pulse_id,))
# #         return [dict(row) for row in cursor.fetchall()]
...
# #         cursor = conn.cursor()
# #         try:
# #             cursor.execute('INSERT INTO pulses (source, title, url, threat_name, threat_category, targeted_industries, targeted_countries, summary, published_at, embedding) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)',
# #                            (pulse_data.get('source'), pulse_data.get('title'), pulse_data.get('url'), pulse_data.get('threat_name'), pulse_data.get('threat_category'), json.dumps(pulse_data.get('targeted_industries', [])), json.dumps(pulse_data.get('targeted_countries', [])), pulse_data.get('summary'), pulse_data.get('published_at'), embedding_blob))
# #             pulse_id = cursor.lastrowid
# #             for ioc in pulse_data.get('indicators', []):
# #                 cursor.execute("INSERT OR IGNORE INTO indicators (type, value, enrichment_data) VALUES (?, ?, ?)", (ioc.get('type'), ioc.get('value'), json.dumps(ioc.get('enrichment'))))
# #                 cursor.execute("SELECT id FROM indicators WHERE value = ?", (ioc.get('value'),))
# #                 indicator_id = cursor.fetchone()[0]
# #                 cursor.execute("INSERT OR IGNORE INTO pulse_indicators (pulse_id, indicator_id) VALUES (?, ?)", (pulse_id, indicator_id))
# #             conn.commit()
# #             print(f"  -> Successfully inserted pulse: {pulse_data.get('title')}")
# #             return pulse_id
# #         except sqlite3.IntegrityError:
# #             # This is expected for duplicate articles, so we don't log it as an error.
# #             return None
# #         except Exception as e:
# #             conn.rollback()
# #             print(f"  ERROR inserting pulse data: {e}")
...
#     with sqlite3.connect(DATABASE_NAME) as conn:
#         cursor = conn.cursor()
#         # Main pulses table with embedding support
#         cursor.execute('''
#             CREATE TABLE IF NOT EXISTS pulses (
#                 id INTEGER PRIMARY KEY AUTOINCREMENT, source TEXT NOT NULL, title TEXT NOT NULL UNIQUE,
...
#             )
#         ''')
#         # Linking table for pulses and indicators
#         cursor.execute('''
#             CREATE TABLE IF NOT EXISTS pulse_indicators (
#                 pulse_id INTEGER, indicator_id INTEGER,
#                 FOREIGN KEY(pulse_id) REFERENCES pulses(id),
...
#             )
#         ''')
#         # --- ADDED: Table for pulse-to-pulse correlations ---
#         cursor.execute('''
#             CREATE TABLE IF NOT EXISTS pulse_correlations (
#                 source_pulse_id INTEGER, related_pulse_id INTEGER, reason TEXT,
#                 FOREIGN KEY(source_pulse_id) REFERENCES pulses(id),
...
#             )
#         ''')
#         # --- ADDED: Table for dynamically discovered sources ---
#         cursor.execute('''
#             CREATE TABLE IF NOT EXISTS dynamic_sources (
#                 domain TEXT PRIMARY KEY,
#                 last_scraped DATETIME,
...
# # --- ADDED: Function to add correlations ---
# def add_correlation(source_id, related_id, reason):
#     if source_id == related_id: return
#     with sqlite3.connect(DATABASE_NAME) as conn:
#         cursor = conn.cursor()
...
# # --- ADDED: Function to get all embeddings for vector search ---
# def get_all_pulses_for_vector_search():
#     with sqlite3.connect(DATABASE_NAME) as conn:
#         cursor = conn.cursor()
#         cursor.execute("SELECT id, embedding FROM pulses WHERE embedding IS NOT NULL")
...
# # --- ADDED: Function to add a new dynamic source ---
# def add_dynamic_source(domain):
#     with sqlite3.connect(DATABASE_NAME) as conn:
#         cursor = conn.cursor()
#         cursor.execute("INSERT OR IGNORE INTO dynamic_sources (domain, last_scraped) VALUES (?, ?)", (domain, None))
...
# # --- MODIFIED: The complete insert function supporting embeddings ---
# def insert_pulse_and_indicators(pulse_data):
...
#                 VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
#             ''', (
#                 pulse_data.get('source'), pulse_title, pulse_data.get('url'),
#                 pulse_data.get('threat_name'), pulse_data.get('threat_category'),
#                 pulse_data.get('severity'), json.dumps(pulse_data.get('targeted_industries', [])),
#                 json.dumps(pulse_data.get('targeted_countries', [])), pulse_data.get('summary'),
#                 pulse_data.get('published_at'), embedding_blob
#             ))
#             pulse_id = cursor.lastrowid
